...
def generate_temp_file(working_dir: Path, extension: str, prefix: str = "temp") -> Path:
    """Generate a unique temporary file path with the given extension.
    
    Args:
        working_dir: Directory where the temp file should be created
        extension: File extension (with or without leading dot)
        prefix: Prefix for the filename
        
    Returns:
        Path to a unique temporary file
    """
    import time
    import os
    ...
    # Create unique identifier using timestamp, PID, and thread ID
    timestamp = int(time.time() * 1000000)  # microsecond timestamp
    pid = os.getpid()
    thread_id = threading.get_ident()

    filename = f"{prefix}_{pid}_{thread_id}_{timestamp}{extension}"
...
class TargetRunner(ABC):
    ...
    @abstractmethod
    def _setup_temp_files(self) -> None:
        """Setup temporary files needed for this runner"""
        pass
    ...
    def measure_executable_size(self) -> int:
        """Measure the size of the primary generated executable/output"""
        if not self._output_files:
            return 0

        primary_output = self._output_files[0]
        if primary_output.exists():
            return primary_output.stat().st_size
        return 0
    ...
    def _add_output_file(self, file_path: Path) -> None:
        """Add a file to the list of output files to be cleaned up"""
        if file_path not in self._output_files:
...
class LuaRunner(TargetRunner):
    ...
    def _setup_temp_files(self) -> None:
        """Setup temporary Lua output file"""
        self.lua_file = generate_temp_file(self.working_dir, "lua", "brad_lua")
        self._add_output_file(self.lua_file)
...
class LuaJitRunner(LuaRunner):
    """Runner for BRAD -> Lua compilation with LuaJIT execution"""
...
class SimpleSpecRunner(TargetRunner):
    ...
    def _setup_temp_files(self) -> None:
        """Setup temporary files for SimpleSpec pipeline output"""
        # SimpleSpec pipeline produces executable and intermediate files
        self.exe_path = generate_temp_file(
            self.working_dir, "out", "brad_simple_spec"
        )
        self._add_output_file(self.exe_path)
        self._add_output_file(Path(f"{self.exe_path}.asm"))
        self._add_output_file(Path(f"{self.exe_path}.o"))

    def get_compilation_command(self, file_path: Path) -> List[str]:
        return [
            "target/release/brad", "v2", "simple-spec-pipeline", "-o", str(self.exe_path), str(file_path)
        ]

    def get_execution_command(self, file_path: Path) -> Optional[List[str]]:
        """Execution command for SimpleSpec pipeline"""
...
class BradBenchmarkRunner:
    ...
    def _get_log_file_path(
        ...
        # Create filename: iteration_001.log or warmup.log
    ...
    def _execute_single_run(
        ...
        try:
            ...
            runner.cleanup_output_files()

            log = self._get_log_file_path(target, file_path, iteration).open('a')
            ...
            executable_size = (
                runner.measure_executable_size()
    ...
    def _create_structured_plots(self, df: pd.DataFrame, output_dir: Path):
        ...
        self._create_cli_comparison_plots(df, plot_dirs["summary"])
        self._create_legacy_plots(df, output_dir)

    def _setup_plot_style(self):
        """Configure matplotlib/seaborn for publication-quality plots"""
        plt.style.use("seaborn-v0_8")
        # Use a distinct palette that works well for multiple CLIs
    ...
    def _create_individual_file_plots(self, df: pd.DataFrame, output_dir: Path):
        ...
        for file_name in df["file"].unique():
            ...
            # Compilation time with logarithmic scaling
            if "compilation_time_s" in file_df.columns and len(file_df) > 1:
                if len(file_df["target"].unique()) > 1:
                    ...
                    # Set log scale if there's significant variation
                    comp_times = file_df["compilation_time_s"].dropna()
                    if comp_times.max() / comp_times.min() > 10 and comp_times.min() > 0:
            ...
            # Execution time with logarithmic scaling
            ...
            if not exec_df.empty and len(exec_df) > 1:
                if len(exec_df["target"].unique()) > 1:
                    ...
                    # Set log scale if there's significant variation
                    exec_times = exec_df["execution_time_s"].dropna()
                    if exec_times.max() / exec_times.min() > 10 and exec_times.min() > 0:
            ...
            # Memory usage with logarithmic scaling
            if "memory_peak_mb" in file_df.columns and len(file_df) > 1:
                if len(file_df["target"].unique()) > 1:
                    ...
                    # Set log scale if there's significant variation
                    memory_vals = file_df["memory_peak_mb"].dropna()
                    memory_vals = memory_vals[memory_vals > 0]  # Only positive values for log scale
                    if len(memory_vals) > 0 and memory_vals.max() / memory_vals.min() > 10:
                        axes[1, 0].set_yscale('log')
                        axes[1, 0].set_ylabel("Memory (MB, log scale)")
                    else:
    ...
    def _create_summary_plots(self, df: pd.DataFrame, output_dir: Path):
        ...
        # Compilation time by file and target with improved visualization
        if not success_df.empty:
            ...
            comp_times = success_df["compilation_time_s"].dropna()
            if comp_times.max() / comp_times.min() > 10 and comp_times.min() > 0:
                axes[0, 0].set_yscale('log')
                axes[0, 0].set_ylabel("Time (seconds, log scale)")
            else:
            ...
            # Apply log scale if needed
            memory_vals = success_df["memory_peak_mb"].dropna()
            memory_vals = memory_vals[memory_vals > 0]
            if len(memory_vals) > 0 and memory_vals.max() / memory_vals.min() > 10:
                axes[1, 0].set_yscale('log')
                axes[1, 0].set_ylabel("Memory (MB, log scale)")
            else:
            ...
            # Performance comparison as grouped bar chart
            summary_stats = (
            ...
            # Create grouped bar chart for better CLI comparison
            if len(summary_stats) > 0:
                pivot_data = summary_stats.pivot(
                    index="file", columns="target", values="compilation_time_s"
                )

                # Use bar plot for better CLI comparison
                pivot_data.plot(kind='bar', ax=axes[1, 1],
                                width=0.8, alpha=0.8)

                # Apply log scale if needed
                if pivot_data.max().max() / pivot_data.min().min() > 10:
                    axes[1, 1].set_yscale('log')
                    axes[1, 1].set_ylabel("Mean Time (seconds, log scale)")
                else:
                    axes[1, 1].set_ylabel("Mean Time (seconds)")

                axes[1, 1].set_title("Mean Compilation Time Comparison")
    ...
    def _create_cli_comparison_plots(self, df: pd.DataFrame, output_dir: Path):
        """Create specialized plots for comparing CLIs side by side"""
        success_df = df[df["success"]]

        if success_df.empty:
            return

        # Create a dedicated CLI comparison figure
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle(
            "CLI Performance Comparison", fontsize=18, fontweight="bold"
        )

        # 1. Side-by-side performance comparison (grouped bar chart)
        if not success_df.empty:
            # Aggregate metrics by target
            metrics_summary = success_df.groupby('target').agg({
                'compilation_time_s': ['mean', 'std'],
            ...
            comp_means = metrics_summary['compilation_time_s_mean']
            comp_stds = metrics_summary['compilation_time_s_std']

            bars1 = axes[0, 0].bar(comp_means.index, comp_means.values,
                                   yerr=comp_stds.values, capsize=5, alpha=0.8)
            axes[0, 0].set_title('Mean Compilation Time by CLI')
            axes[0, 0].set_ylabel('Time (seconds)')
            axes[0, 0].tick_params(axis='x', rotation=45)
            axes[0, 0].grid(True, alpha=0.3)

            # Apply log scale if needed
            if comp_means.max() / comp_means.min() > 10 and comp_means.min() > 0:
                axes[0, 0].set_yscale('log')
                axes[0, 0].set_ylabel('Time (seconds, log scale)')

            # Add value labels on bars
            for bar, mean_val in zip(bars1, comp_means.values):
        ...
        if 'memory_peak_mb' in success_df.columns:
            memory_data = success_df.groupby('target')['memory_peak_mb'].agg(['mean', 'std'])

            bars2 = axes[0, 1].bar(memory_data.index, memory_data['mean'],
                                   yerr=memory_data['std'], capsize=5, alpha=0.8)
            axes[0, 1].set_title('Mean Memory Usage by CLI')
            axes[0, 1].set_ylabel('Memory (MB)')
            axes[0, 1].tick_params(axis='x', rotation=45)
            axes[0, 1].grid(True, alpha=0.3)

            # Apply log scale if needed
            if memory_data['mean'].max() / memory_data['mean'].min() > 10 and memory_data['mean'].min() > 0:
                axes[0, 1].set_yscale('log')
                axes[0, 1].set_ylabel('Memory (MB, log scale)')

            # Add value labels
            for bar, mean_val in zip(bars2, memory_data['mean']):
                height = bar.get_height()
                axes[0, 1].text(bar.get_x() + bar.get_width() / 2., height,
                                f'{mean_val:.1f}MB', ha='center', va='bottom', fontsize=9)

        # 3. Performance vs consistency scatter plot
        if not success_df.empty:
            perf_consistency = success_df.groupby('target').agg({
                'compilation_time_s': ['mean', 'std']
            })
            perf_consistency.columns = ['mean_time', 'std_time']
            perf_consistency['cv'] = (perf_consistency['std_time'] / perf_consistency['mean_time']) * 100

            axes[1, 0].scatter(perf_consistency['mean_time'], perf_consistency['cv'],
                               s=100, alpha=0.7)

            # Add labels for each point
            for idx, row in perf_consistency.iterrows():
                axes[1, 0].annotate(idx, (row['mean_time'], row['cv']),
                                    xytext=(5, 5), textcoords='offset points', fontsize=10)

            axes[1, 0].set_xlabel('Mean Compilation Time (s)')
            axes[1, 0].set_ylabel('Coefficient of Variation (%)')
            axes[1, 0].set_title('Performance vs Consistency')
            axes[1, 0].grid(True, alpha=0.3)

            # Apply log scale to x-axis if needed
            if perf_consistency['mean_time'].max() / perf_consistency['mean_time'].min() > 10:
                axes[1, 0].set_xscale('log')
                axes[1, 0].set_xlabel('Mean Compilation Time (s, log scale)')

        # 4. Success rate comparison
        success_rates = df.groupby('target')['success'].agg(['mean', 'count'])
        success_rates['success_rate'] = success_rates['mean'] * 100

        colors = ['green' if x >= 90 else 'orange' if x >= 70 else 'red'
                  for x in success_rates['success_rate']]

        bars4 = axes[1, 1].bar(success_rates.index, success_rates['success_rate'],
                               color=colors, alpha=0.8)
        axes[1, 1].set_title('Success Rate by CLI')
        axes[1, 1].set_ylabel('Success Rate (%)')
        axes[1, 1].set_ylim(0, 100)
        axes[1, 1].tick_params(axis='x', rotation=45)
        axes[1, 1].grid(True, alpha=0.3)

        # Add value labels and sample count
        for bar, (idx, row) in zip(bars4, success_rates.iterrows()):
            height = bar.get_height()
        ...
        plt.tight_layout()
        plt.savefig(output_dir / "cli_comparison.png", dpi=300, bbox_inches="tight")
        plt.savefig(output_dir / "cli_comparison.pdf", bbox_inches="tight")
        plt.close()
    ...
    def _create_legacy_plots(self, df: pd.DataFrame, output_dir: Path):
        ...
        # Execution time boxplot with logarithmic scaling
        if (
            ...
            if not execution_df.empty:
                ...
                # Apply log scale if there's significant variation
                exec_times = execution_df["execution_time_s"].dropna()
                if exec_times.max() / exec_times.min() > 10 and exec_times.min() > 0:
                    axes[0, 1].set_yscale('log')
                    axes[0, 1].set_ylabel("Time (seconds, log scale)")
                else:
        ...
        # Memory usage boxplot with logarithmic scaling
        if "memory_peak_mb" in success_df.columns:
        ...
        # Executable size comparison with logarithmic scaling
        if (
            ...
            if not size_df.empty:
                ...
                # Apply log scale if there's significant variation
                size_vals = size_df["executable_size_bytes"].dropna()
                if size_vals.max() / size_vals.min() > 10 and size_vals.min() > 0:
                    axes[1, 1].set_yscale('log')
                    axes[1, 1].set_ylabel("Size (bytes, log scale)")
                else:
